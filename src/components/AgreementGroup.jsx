...
const AgreementGroup = () => {
  ...
  const links = [
    { to: "/agreement", label: "Terms and Conditions" },
    { to: "/authoritytoact", label: "Authority to Act" },
    { to: "/authoritytoallow", label: "Authority to Allow" },
    { to: "/authoritytoshare", label: "Authority to Share " },

    // Add more links as needed
  ];

  const [activeLink, setActiveLink] = useState("/agreement");
  ...
  const handleNext = ()=>{
    ...
    navigate(
      app?.isAuthority
        ...
        ? "/authoritytoallow"
        : app?.isShareClientDetails ? "/authoritytoshare" : "/group/phase2"
    );
  }
