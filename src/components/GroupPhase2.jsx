...
import { useGetGroupClientAppByUserIdQuery, usePostGroupClientPhase2Mutation } from "../services/api/companyClient";
...
const GroupPhase2 = () => {
  ...
  const { data, isLoading } = useGetGroupClientAppByUserIdQuery();
  const application = data?.application;
  console.log(data);

  const [postGroupClientPhase2, result] = usePostGroupClientPhase2Mutation();
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    if (!application || !application._id) {
      console.error("Invalid application object or application._id is missing");
      return;
    }
    try {
      console.log("form data", formData);
      const { data: resp } = await postGroupClientPhase2(formData);
      resetForm({
